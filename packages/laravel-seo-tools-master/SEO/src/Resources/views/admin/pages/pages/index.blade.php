...
&nbsp;&nbsp;
    <div class="btn-group">
        ...
        <a href="{{route('pages.download')}}" class="btn btn-outline-secondary">
            <i class="fa fa-download"></i> Download CSV
        </a>
        <a href="{{route('pages.zip')}}" class="btn btn-outline-secondary">
            <i class="fa fa-file-zip-o"></i> Download Zip
        </a>
        @if(config('seo.cache.enable'))
            <form action="{{route('pages.cache')}}" method="post" style="display: inline">
                {{csrf_field()}}
    ...
    <div class="row">
        @foreach($records as $record)
            <div class="col-sm-6">
                @include('seo::admin.cards.page')
            </div>
        @endforeach
    </div>
    {!! $records->render() !!}
    @include('seo::admin.modals.page_upload')
@endSection